...
public:
	...
	Platform_information(CString sub_directory_name)
	{
		finder_result=false;
		...
		directory_name+=sub_directory_name+_T("\\");
...
*/
	...
	inline static void backupdata()
	{
		...
		if(dlg.DoModal()==IDOK){
			...
			::ShellExecute(NULL,_T("open"),_T("WinRAR"),arg,get_directory_name(),SW_SHOWNORMAL);
		}
	}
	...
	inline static void restoredata()
	{
		...
		if(dlg.DoModal()==IDOK){
			set_file_attribute(get_directory_name());
			CString arg=_T( "x -y -r \"")+(dlg.GetPathName())+_T("\"");
			run_wait_program(_T("WinRAR"),arg,get_directory_name(),SW_SHOWNORMAL);
		}
	}
	inline static void set_file_attribute(CString strDir)
	{   
		...
		while(bFound){ 
			...
			if(!(ff.IsDots())){
				if(ff.IsDirectory())
					set_file_attribute(ff.GetFilePath());
				else
	...
	inline static void delete_directory(CString strDir)
	{   
		CFileFind ff; 
		BOOL bFound=ff.FindFile(strDir+_T("\\*")); 
		while(bFound){ 
			...
			if(!(ff.IsDots())){
				if(ff.IsDirectory())
					delete_directory(ff.GetFilePath()); 
				else 
	...
	inline static bool delete_all_data()
	{
		if(AfxMessageBox(_T("首次确认删除所有拼装内容?"),MB_YESNO)!=IDYES)
			return false;
		if(AfxMessageBox(_T("再次确认删除所有拼装内容?"),MB_YESNO)!=IDYES)
			return false;
		if(AfxMessageBox(_T("三次确认删除所有拼装内容?"),MB_YESNO)!=IDYES)
			return false;
		CFileFind finder;
		BOOL bWorking = finder.FindFile(get_directory_name()+_T("*"));
		while (bWorking){
			bWorking = finder.FindNextFile();
			if(!(finder.IsDots()))
				if(finder.IsDirectory()){
					set_file_attribute(finder.GetFilePath());
					delete_directory(finder.GetFilePath());
				}
		}
		finder.Close();
