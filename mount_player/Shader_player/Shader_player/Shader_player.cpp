...
BOOL CShader_playerApp::InitInstance()
{
	...
	while((File_data::get_file_length(str+part_file_name)<=0)||(File_data::get_file_length(str+configure_file_name)<=0)){
		if(AfxMessageBox(_T("还没有安装相关拼装内容\n选择拼装内容文件进行安装?"),MB_YESNO)!=IDYES)
			return FALSE;
...
{
	if(ws!=NULL){
		if(ws->plat_info!=NULL){
			if(ws->plat_info->get_last_time()>CTime(2011,12,31,23,59,59)){
				CString str;
				str =_T("此软件为试用版，软件试用期结束\n");
				str+=_T("请联系北京铁路局人民武装部购买正版软件\n");
				str+=_T("联系人:张金明，联系电话：010-51822218\n");
				AfxMessageBox(str);
				ws->exit_flag=true;
			}
		}
