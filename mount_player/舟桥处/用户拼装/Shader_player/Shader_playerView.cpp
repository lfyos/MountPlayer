...
BOOL CShader_playerView::OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
	...
	int mouse_wheel_id=(Platform_information::get_function_key_state()>=4)?0:(theApp.mouse_wheel_id);
...
{
	User_interface(*(theApp.ws),*gs,CPaintDC(this),camera_id).set_component_transparence(0.75,true);
	theApp.redraw_flag=true;
}
...
void CShader_playerView::OnBackupData()
{
	...
	CString directory_name=Platform_information::get_directory_name();
	
	CString arg=_T(" a  -r -x*.exe  -x*.dll ")+file_name+_T(" *.*");

	::ShellExecute(NULL,_T("open"),_T("WinRAR.exe"),arg,directory_name,SW_SHOWNORMAL);
}
void CShader_playerView::OnBackupCurrentData()
{
	...
	CString file_name=dlg.GetPathName();
	CString directory_name=Platform_information::get_directory_name();
	CString current_director_name=theApp.ws->plat_info->directory_name;
	int length=current_director_name.GetLength()-directory_name.GetLength();
	
	CString arg=_T(" a  -r -x*.exe  -x*.dll ")+file_name+_T(" ")+current_director_name.Right(length);
...
void CShader_playerView::OnRestoreData()
{
	...
	if(dlg.DoModal()!=IDOK)
		return;
	CString file_name=dlg.GetPathName(),directory_name=Platform_information::get_directory_name();
	CString arg=_T( "x -y -r ")+file_name+_T(" *.*");
	Platform_information::run_wait_program(_T("WinRAR.exe"),arg,directory_name,SW_SHOWNORMAL);

	theApp.mounter->push_movement();
	theApp.mounter->load();
	pFrame->m_movement_list.do_file_list();
}