...
BOOL CShader_playerApp::InitInstance()
{
	...
	str=str+sub_directory_name+_T("\\")+part_file_name;

	if(File_data::get_file_length(str)<=0){
		AfxMessageBox(_T("没有安装相关模块"));
		return FALSE;
	}
...
BOOL CShader_playerApp::OnIdle(LONG lCount)
{
	if(ws!=NULL){
		if(ws->plat_info!=NULL){
			if(ws->plat_info->get_last_time()>CTime(2013,12,31,23,59,59)){
				bool flag=true;
				if(flag){
					flag=false;
